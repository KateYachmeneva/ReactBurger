...
function Profile(props) {
  ...
  const [lockedFields, setLockedFields] = useState(['name', 'email', 'password'])
  const resetForm = () => {
    setValues({
      email: user.email,
      name: user.name,
  ...
  const toggleField= (name) => {
    setLockedFields(lockedFields.includes(name) ? lockedFields.filter(fieldName => fieldName !== name) : [...lockedFields, name])
  }
